"""Enterprise Task Management & Executive Dashboard — Compact Edition"""
import streamlit as st, pandas as pd, numpy as np, sqlite3, plotly.express as px
from datetime import datetime, timedelta
import random, io, os

st.set_page_config("Task Management Dashboard", "📊", layout="wide")
DB = "company_tasks.db"
...
def log(user, action, task_id, desc):
    c = conn(); c.execute("INSERT INTO logs(Timestamp,User,Action,Task_ID,Description) VALUES(?,?,?,?,?)",
        (datetime.now().isoformat(sep=" ", timespec="seconds"), user, action, task_id, desc))
    c.commit(); c.close()
...
def seed():
    ...
    if pd.read_sql("SELECT COUNT(*) n FROM tasks", c).iloc[0]["n"] == 0:
        ...
        c.executemany("INSERT INTO tasks VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                       gen_tasks(200, staff))
        c.commit(); log("System","Seed","-","Initial data generated")
    c.close()

def load(table):
    c = conn(); df = pd.read_sql(f"SELECT * FROM {table}", c); c.close()
...
def staff_metrics(tasks, staff):
    ...
    for _, s in staff.iterrows():
        ...
        rows.append({"Staff_Name": s["Staff_Name"], "Sector": s["Sector"], "Team": s["Team"],
            "Assigned": assigned, "Completed": completed, "Overdue": overdue,
            "Utilization_%": util, "Productivity_%": prod, "Workload": wc})
...
page = st.sidebar.radio("Navigate", ["Executive Dashboard","Live Monitor","Task Management",
    "Staff Management","Project Analytics","Reports","Activity Logs"])
...
if page == "Executive Dashboard":
    ...
    kpi(st.columns(5), [("Total", total), ("Completed", completed), ("Active", active),
        ("Overdue", overdue), ("Completion %", f"{round(completed/total*100,1) if total else 0}%")])
    c1, c2 = st.columns(2)
    ...
    st.plotly_chart(px.bar(tasks_df["Sector"].value_counts().reset_index(),
        x="count", y="Sector", orientation="h", title="Tasks by Sector"), use_container_width=True)
...
# ---------- LIVE MONITOR ----------
    ...
    live_df = tasks_df[~tasks_df.Effective_Status.isin(["Completed","Cancelled"])]
    order = {"Critical":0,"Overdue":1,"Warning":2,"Upcoming":3,"Normal":4}
    live_df = live_df.sort_values(by="Alert", key=lambda s: s.map(order))
    kpi(st.columns(4), [("Critical", len(live_df[live_df.Alert=="Critical"])),
        ("Warning", len(live_df[live_df.Alert=="Warning"])),
        ("Upcoming", len(live_df[live_df.Alert=="Upcoming"])),
        ("Overdue", len(live_df[live_df.Alert=="Overdue"]))])
    for _, r in live_df.head(40).iterrows():
        with st.container(border=True):
            c1, c2 = st.columns([3,2])
            c1.write(f"**{r.Task_Name}** ({r.Task_ID}) — {r.Priority} — {r.Alert}")
            c1.write(f"👤 {r.Assigned_Person} | {r.Current_Process}")
            c2.progress(min(100,max(0,int(r.Progress_Percentage)))/100, text=f"{r.Progress_Percentage}%")
            c2.caption(f"Deadline: {r.End_DateTime} | {r.Remaining_Hours:.1f}h left")

# ---------- TASK MANAGEMENT ----------
elif page == "Task Management":
    st.title("🗂️ Task Management")
    tab1, tab2, tab3 = st.tabs(["Add", "Edit/Delete", "All Tasks"])

    with tab1:
        with st.form("add", clear_on_submit=True):
            name = st.text_input("Task Name*")
            sector = st.selectbox("Sector", SECTORS)
            person = st.selectbox("Assigned Person", sorted(staff_df.Staff_Name))
            project = st.selectbox("Project", PROJECTS)
            priority = st.selectbox("Priority", PRIORITIES)
            status = st.selectbox("Status", STATUSES)
            progress = st.slider("Progress %", 0, 100, 0)
            c1, c2 = st.columns(2)
            sd = c1.date_input("Start Date"); st_t = c1.time_input("Start Time")
            ed = c2.date_input("End Date", value=datetime.now()+timedelta(days=3)); et = c2.time_input("End Time")
            est = st.number_input("Estimated Hours", min_value=0.5, value=8.0)
            if st.form_submit_button("Add Task"):
                s_dt, e_dt = datetime.combine(sd, st_t), datetime.combine(ed, et)
                if not name.strip(): st.error("Task name required")
                    ...
                    c.execute("""INSERT INTO tasks VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        (tid, name, name, person, TEAMS[0], sector+" Dept", sector, project,
                         TASK_TYPES[0], priority, status, progress, PROCESS_STAGES[0],
                         s_dt.isoformat(sep=" ",timespec="minutes"), e_dt.isoformat(sep=" ",timespec="minutes"),
                         est, 0.0, level(progress), CLIENTS[0], "-", datetime.now().isoformat(sep=" ",timespec="minutes")))
                    c.commit(); c.close()
    ...
    with tab2:
        ...
        with st.form("edit"):
            ...
            c1, c2 = st.columns(2)
            upd = c1.form_submit_button("💾 Update"); dele = c2.form_submit_button("🗑️ Delete")
            if upd:
            ...
            if dele:
                ...
                log(st.session_state.user, "Task Deleted", tid, "Removed")
                st.warning("Deleted"); st.rerun()

    with tab3:
        st.dataframe(tasks_df[["Task_ID","Task_Name","Assigned_Person","Sector","Priority",
            "Effective_Status","Progress_Percentage","End_DateTime"]], use_container_width=True, height=400)
        export(tasks_df, "all_tasks")
...
# ---------- STAFF ----------
    ...
    with st.form("add_staff", clear_on_submit=True):
        ...
        hours = st.number_input("Working Hours", value=160.0)
        if st.form_submit_button("Add Staff"):
            if name.strip():
                ...
                c.commit(); c.close(); log(st.session_state.user,"Staff Added","-",name)
                st.success("Added"); st.rerun()
    st.dataframe(metrics_df, use_container_width=True, height=400)
    export(metrics_df, "staff_metrics")
    st.plotly_chart(px.scatter(metrics_df, x="Utilization_%", y="Productivity_%", size="Assigned",
        color="Workload", hover_name="Staff_Name"), use_container_width=True)
...
# ---------- PROJECT ANALYTICS ----------
    ...
    export(pdf, "project_analytics")

# ---------- REPORTS ----------
elif page == "Reports":
    st.title("📑 Reports")
    st.write(f"Total Tasks: **{len(tasks_df)}**, Completed: **{len(tasks_df[tasks_df.Effective_Status=='Completed'])}**")
    dept_perf = tasks_df.groupby("Sector").agg(Total=("Task_ID","count"),
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        ...
        dept_perf.to_excel(w, index=False, sheet_name="Sector")
    st.download_button("⬇️ Executive Summary (Excel)", buf.getvalue(), "summary.xlsx")

# ---------- LOGS ----------
elif page == "Activity Logs":
    st.title("📜 Activity Logs")
    st.dataframe(load("logs").sort_values("Log_ID", ascending=False), use_container_width=True, height=450)
